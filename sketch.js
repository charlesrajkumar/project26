...
function handlePlayerArrowCollision() {
  ...
  for (var i = 0; i<playerArrows.length;i++) {
    var baseCollition = Matter.SAT.collides(   playerArrows[i].body,   computerBase.body  );

    var archerCollition = Matter.SAT.collides(
      playerArrows[i].body,
      computerArcher.body
    );

    var computerCollition = Matter.SAT.collides(
      playerArrows[i].body,
      computer.body
    );

    if(   baseCollition.collided ||   archerCollition.collided ||   computerCollition.collided   )
    {
...
function handleComputerArrowCollision() {
  ...
  for (var i = 0; i<computerArrows.length;i++) {
    var baseCollition = Matter.SAT.collides(   computerArrows[i].body,   playerBase.body  );
